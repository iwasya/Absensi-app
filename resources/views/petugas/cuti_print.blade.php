...
<head>
    <meta charset="UTF-8">
    <title>Surat Izin Cuti - {{ $cuti->user->nama }}</title>
    <style>
        @page {
            size: A4;
...
<body onload="window.print()">
    ...
    <div class="nomor-surat">
        <h3>SURAT IZIN {{ strtoupper($cuti->jenis_cuti) }}</h3>
        <p>Nomor: {{ date('Y') }}/SC-DLH/{{ $cuti->id_cuti }}</p>
    </div>

    <div class="content">
        <p>Yang bertanda tangan di bawah ini, menerangkan bahwa:</p>
        <table class="data-table">
            <tr><td>Nama</td><td>: <strong>{{ $cuti->user->nama }}</strong></td></tr>
            <tr><td>NIK / Username</td><td>: {{ $cuti->user->username }}</td></tr>
            <tr><td>Unit Kerja</td><td>: {{ $cuti->user->tempatTugas->nama_tempat ?? '-' }}</td></tr>
            <tr><td>Jabatan</td><td>: {{ $cuti->user->role->nama_role ?? '-' }}</td></tr>
        </table>
        ...
        <table class="data-table">
            ...
            <tr><td>Mulai Tanggal</td><td>: {{ $cuti->tanggal_mulai->translatedFormat('d F Y') }}</td></tr>
    ...
    <div class="signature-wrapper">
        <div class="signature-box">
            ...
            <p><strong>{{ $cuti->pengganti->nama ?? '..........................' }}</strong></p>
        </div>
        ...
        <div class="signature-box">
            ...
            <p><strong>{{ $cuti->approver->nama ?? '..........................' }}</strong></p>
        </div>
    </div>
    ...
    <div class="signature-wrapper" style="justify-content: center; margin-top: 30px;">
        <div class="signature-box">
            ...
            <p><strong>{{ $cuti->user->nama }}</strong></p>
        </div>
    </div>
